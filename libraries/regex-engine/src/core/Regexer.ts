...
import { parse } from "@core/RegexParser";
...
export class Regexer
{
    ...
    public async parse(regexString : string = "")
    {
        try{
            ...
            const exception = e as peggy.GrammarError;
            const errorCode = Number.parseInt(exception.message) as RegexTypes.RegexParserErrors;

            throw new RegParseException(exception?.location?.start?.offset ?? 0, exception?.location?.end?.offset ?? 0, errorCode);
        }
